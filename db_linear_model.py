...
'''
usage: $0 soft_db*csv,xlsx,bmf,vtk soft_condition hard_db*csv,xlsx,bmf,vtk hard_condition xyz#field:hard_db lito:hard_db variables#variable:hard_db engine%kn,rn,rn+kn kn_neighbors rn_radius mode%full,fill output*csv,xlsx,bmf,vtk display@
'''
...
def r_predict_workaround(r, radius, df0v, df1v, v_type):
  ...
  d = np.full(len(df0v), np.nan, dtype=v_type)
  if np.any(bi):
...
def pd_linear_model(df0, df1, df1_xyz, variable, fill = False, engine = None, radius = None, n_neighbors = None, weights = None):
  ...
  if sys.hexversion < 0x3080000 and engine.startswith('rn'):
    weights = None
  ...
  if len(df1v):
    ...
    #  rmat = r_predict_workaround(r, radius, df0v, df1v[df1_xyz], df1[variable].dtype)
    #else:
    rmat = r.predict(df0v.values)

    if fill and variable in df0:

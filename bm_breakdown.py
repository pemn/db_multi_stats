...
'''
usage: $0 input*bmf,csv,xlsx,json,isis,dm,tif,tiff,00t,obj,vtk condition variables#variable:input#type=breakdown,count,sum,mean,min,max,var,std,sem,q1,q2,q3,p10,p90,major,list,rank,text,group,distinct,cv#weight:input keep_null@ output*csv,xlsx
'''
'''
...
def weighted_mean(df, name, wt):
  if len(wt):
    ...
    if np.nansum(ws) != 0:
      # weighted mean
      return np.average(df.loc[bi, name].values.astype(np.float_), None, np.nan_to_num(ws))

  return pd.Series.mean(df[name])
...
def main(*args):
  return pd_save_dataframe(bm_breakdown(args[0], args[1], args[2], args[3]), args[4])
